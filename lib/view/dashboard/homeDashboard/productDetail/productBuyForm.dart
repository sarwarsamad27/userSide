...
/// Payment method enum
enum PaymentMethod { cod, wallet, jazzcash }
...
class _ProductBuyFormState extends State<ProductBuyForm> {
  ...
  final TextEditingController _jazzcashPhoneController = TextEditingController();
  ...
  List<Map<String, dynamic>> _buildProductList() {
    if (widget.favouriteItems != null && widget.favouriteItems!.isNotEmpty) {
      return widget.favouriteItems!.map((item) => {
  ...
  double _getGrandTotal() {
    final isFromFavourite = widget.favouriteItems != null && widget.favouriteItems!.isNotEmpty;
  ...
  Future<bool> _showWalletPhoneSheet(double amount, CreateOrderProvider provider) async {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // JAZZCASH: Phone sheet → initiate → wait for app confirm → place order
  // ──────────────────────────────────────────────────────────────────────────
  Future<void> _startJazzcashFlow() async {
    final grandTotal = _getGrandTotal();
    final provider   = context.read<CreateOrderProvider>();

    // ── Step 1: Ask JazzCash number + initiate ────────────────────────────────
    _jazzcashPhoneController.text = _phoneController.text.trim();
    final initiated = await _showJazzcashInitiateSheet(grandTotal, provider);
    if (!initiated) return;

    // ── Step 2: Wait for user to approve in JazzCash app, then confirm ────────
    final confirmed = await _showJazzcashWaitSheet(provider);
    if (!confirmed) return;
  ...
  Future<bool> _showJazzcashInitiateSheet(double amount, CreateOrderProvider provider) async {
    final result = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _JazzcashInitiateSheet(
        amount:     amount,
        controller: _jazzcashPhoneController,
        provider:   provider,
        onInitiated: () => Navigator.pop(context, true),
        onCancel:    () => Navigator.pop(context, false),
      ),
    );
    return result ?? false;
  ...
  Future<bool> _showJazzcashWaitSheet(CreateOrderProvider provider) async {
    final result = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _JazzcashWaitSheet(
        provider:   provider,
  ...
  void _handleOrderResult(CreateOrderProvider provider) {
    if (provider.orderData != null && provider.orderData!.order != null) {
      ...
      if (mounted) {
        PremiumToast.error(context, provider.errorMessage ?? 'Failed to place order');
      }
    }
  }
          ...
          Scaffold(
            ...
            bottomNavigationBar: SafeArea(
              child: Padding(
                ...
                child: CustomButton(text: 'Place Order', onTap: _onPlaceOrderTapped),
              ),
            ),
                        ...
                        Row(
                          children: [
                            ClipRRect(
                              borderRadius: BorderRadius.circular(10.r),
                              child: Image.network(
                                widget.imageUrl!, height: 60.h, width: 60.w, fit: BoxFit.cover,
                              ),
                            ),
                            SizedBox(width: 12.w),
                            Expanded(
                        ...
                        ValueListenableBuilder<int>(
                          ...
                          builder: (_, quantity, __) => Row(
                            children: [
                              ...
                              SizedBox(width: 20.w),
                              _qtyBtn(Icons.add, () => _quantityNotifier.value++),
                            ],
                          ),
                        ),
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: _PaymentOptionCard(
                                      ...
                                      onTap:    () => setState(() => _selectedPayment = PaymentMethod.wallet),
                                    ),
                                  ),
                                  SizedBox(width: 10.w),
                                  Expanded(
                                    child: _PaymentOptionCard(
                                      imagePath: 'assets/images/JazzCashLogo.jpg',
                                      label:    'JazzCash',
                                      color:    const Color(0xFFFF6F00),
                                      selected: _selectedPayment == PaymentMethod.jazzcash,
                                      onTap:    () => setState(() => _selectedPayment = PaymentMethod.jazzcash),
                                    ),
                                  ),
                                ],
                              ),
                                  ...
                                  ValueListenableBuilder<int>(
                                    ...
                                    builder: (_, quantity, __) {
                                      ...
                                      return Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text('Total Price: Rs ${productTotal.toStringAsFixed(0)}',
                                            style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.bold)),
                                          SizedBox(height: 8.h),
                                          Text('Shipment Charges: Rs 200',
                                            style: TextStyle(fontSize: 14.sp, color: Colors.black87)),
                                          SizedBox(height: 8.h),
                                          Text('Grand Total: Rs ${grandTotal.toStringAsFixed(0)}',
          ...
          ValueListenableBuilder<bool>(
            ...
            builder: (_, loading, __) {
              ...
              return Container(
                ...
                child: Center(
                  child: SpinKitThreeBounce(color: AppColor.primaryColor, size: 40),
                ),
              );
            },
          ),
  ...
  String _paymentLabel() {
    switch (_selectedPayment) {
      ...
      case PaymentMethod.wallet:   return '💳 Pay from Wallet';
      case PaymentMethod.jazzcash: return '📱 Pay via JazzCash';
    }
  }

  Color _paymentColor() {
    switch (_selectedPayment) {
      case PaymentMethod.cod:      return const Color(0xFF4CAF50);
      case PaymentMethod.wallet:   return const Color(0xFF2979FF);
      case PaymentMethod.jazzcash: return const Color(0xFFFF6F00);
    }
  }
          ...
          children: [
            if (imagePath != null)
              ClipRRect(
                ...
                child: Image.asset(imagePath!, width: 28.w, height: 28.h, fit: BoxFit.cover),
              )
            else
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Icon(Icons.account_balance_wallet_rounded,
                color: const Color(0xFF2979FF), size: 24.r),
              SizedBox(width: 8.w),
              Text('Wallet Payment',
          ...
          Consumer<CreateOrderProvider>(
            builder: (_, provider, __) => SizedBox(
              ...
              child: ElevatedButton(
                onPressed: provider.walletOtpLoading ? null : () async {
                  final phone = widget.controller.text.trim();
                  if (phone.isEmpty || !RegExp(r'^03[0-9]{9}$').hasMatch(phone)) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Enter a valid Pakistani number (03XXXXXXXXX)')),
                    );
                    return;
                  }
        ...
        children: [
          ...
          Consumer<CreateOrderProvider>(
            builder: (_, provider, __) => SizedBox(
              ...
              height: 50.h,
              child: ElevatedButton(
                onPressed: provider.walletVerifyLoading ? null : () async {
                  ...
                  final verified = await provider.verifyWalletOtp(otp: otp);
                  if (verified) {
                    widget.onVerified();
                  } else if (provider.errorMessage != null) {
                    ScaffoldMessenger.of(context).showSnackBar(
        ...
        children: [
            ...
            children: [
              ClipRRect(
                ...
                child: Image.asset('assets/images/JazzCashLogo.jpg',
                  width: 32.w, height: 32.h, fit: BoxFit.cover),
              ),
              SizedBox(width: 10.w),
          ...
          Consumer<CreateOrderProvider>(
            builder: (_, provider, __) => SizedBox(
              ...
              height: 50.h,
              child: ElevatedButton(
                onPressed: provider.jazzcashLoading ? null : () async {
                  final mobile = widget.controller.text.trim();
                  if (mobile.isEmpty || !RegExp(r'^03[0-9]{9}$').hasMatch(mobile)) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Enter a valid JazzCash number (03XXXXXXXXX)')),
                    );
                    return;
                  }
        ...
        children: [
          ...
          Container(
            ...
            child: Icon(Icons.phone_android_rounded,
              color: const Color(0xFFFF6F00), size: 40.r),
          ),
          SizedBox(height: 16.h),
          Text('Open JazzCash App',
            style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.w700)),
          SizedBox(height: 8.h),
          Text(
          ...
          Consumer<CreateOrderProvider>(
            builder: (_, prov, __) => SizedBox(
              ...
              height: 50.h,
              child: ElevatedButton(
                onPressed: prov.jazzcashLoading ? null : () async {
